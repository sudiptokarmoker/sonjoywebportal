...
class SongsRepository implements SongsRepositoryInterface
{
    private $songs_category_id = 2;
    public function all()
    {
        return PostsModel::where('root_category_id', '=', $this->songs_category_id)->
            orderBy("id", "asc")
            ->get()
            ->map(function ($row) {
                return [
    ...
    {
        $getSongsCategorryId = PostsCategoryModel::where('category_name', 'songs')->first();
        return $getSongsCategorryId ? $getSongsCategorryId->id : null;
    }
    public function getAllSongsCategoryLists()
    {
        $getSongsCategorryId = PostsCategoryModel::where('category_name', 'songs')->first();
        $collection = PostsCategoryModel::where('parent_id', $getSongsCategorryId->id)->get();
    ...
    public function edit($id)
    {
        ...
        return [
            ...
            'category_lists' => $this->getAllSongsCategoryLists(),
